...
contract ReleaseRegistryTest is GifDeployer, FoundryRandom {
    ...
    function _getAndVerifyServiceInfoChecks(IService service, address expectedOwner, VersionPart expectedVersion) 
        public 
        view
        returns(bool expectRevert, bytes memory revertMsg)
    {
        ...
        if(info.objectAddress != address(service)) {
            ...
            expectRevert = true;
        } else if(info.release != expectedVersion) {
            revertMsg = abi.encode(
