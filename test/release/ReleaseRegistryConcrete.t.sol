...
contract ReleaseRegistryConcreteTest is GifDeployer, FoundryRandom {
    ...
    {
        for(uint8 i = 0; i <= GIF_INITIAL_RELEASE().toInt() + 1; i++) {
        ...
        assertEq(releaseRegistry.getNextVersion().toInt(), GIF_INITIAL_RELEASE().toInt() - 1, "getNextVersion() return unexpected value");
    ...
    {
        VersionPart newReleaseVersion = VersionPartLib.toVersionPart(GIF_INITIAL_RELEASE().toInt());
        VersionPart previousReleaseVersion = VersionPartLib.toVersionPart(GIF_INITIAL_RELEASE().toInt() - 1);
    ...
    {
        VersionPart nextVersion = VersionPartLib.toVersionPart(GIF_INITIAL_RELEASE().toInt());
        VersionPart prevVersion = VersionPartLib.toVersionPart(GIF_INITIAL_RELEASE().toInt() - 1);
    ...
    function test_releaseRegistry_createRelease_whenReleaseDeployingHappyCase() public 
    {
        ...
        for(uint256 i = 0; i <= 10; i++) 
        {
                ...
                if(i > 0) {
                    ...
                    assertTrue(
                        AccessManagerCloneable(
                            ReleaseAdmin(
                                prevReleaseInfo.releaseAdmin).authority()).isLocked(), 
...

            {
                ...
                nextReleaseInfo.releaseAdmin = address(preparedAdmin);
...
console.log("block 3g, i:", i);
            ...
            {
                // solhint-disable-next-line
                console.log("activating release", i);
                ...
                // solhint-disable-next-line
                console.log("activated release", i);
            }
        ...
        {
            {
                ...
                assertEq(createdVersion.toInt(), nextVersion.toInt(), "createNextRelease() return unexpected value");
                if(i > 0) {
                    assertFalse(AccessManagerCloneable(ReleaseAdmin(prevReleaseInfo.releaseAdmin)
                    .authority()).isLocked(), "isLocked() return unexpected value");
                }
            ...
            {
                // deploy (register all(1) services)
                service = _prepareServiceWithRegistryDomain(nextReleaseInfo.version, ReleaseAdmin(address(ReleaseAdmin(nextReleaseInfo.releaseAdmin)
                    )));
                assertFalse(registry.isRegisteredService(address(service)), "isRegisteredService() return unexpected value #1");
    ...
    {
        VersionPart releaseVersion = VersionPartLib.toVersionPart(GIF_INITIAL_RELEASE().toInt());
    ...
    {
        VersionPart expectedVersion = VersionPartLib.toVersionPart(GIF_INITIAL_RELEASE().toInt());
    ...
    function test_releaseRegistry_prepareRelease_initialReleaseHappyCase() public
    {
        ...
        _assert_releaseRegistry_getters(
            VersionPartLib.toVersionPart(GIF_INITIAL_RELEASE().toInt() - 1),
            zeroReleaseInfo());
    ...
    */
    ...
    {
        NftOwnableMock mock = new NftOwnableMock();
    ...
    {
        uint8 createdReleases = uint8(randomNumber(2, 10));
        uint8 releaseVersion = GIF_INITIAL_RELEASE().toInt() + createdReleases;
        ...
        for(uint8 i = 0; i < releaseVersion; i++) 
        {
    ...
    {
        uint8 createdReleases = uint8(randomNumber(1, 10));
        uint8 releaseVersion = GIF_INITIAL_RELEASE().toInt() + createdReleases;
        ...
        for(uint8 i = releaseVersion + 1; i < releaseVersion + 5; i++) 
        {
    ...
    {
        VersionPart version = VersionPartLib.toVersionPart(GIF_INITIAL_RELEASE().toInt());
    ...
    function test_releaseRegistry_registerService_whenReleaseDeploying_withServiceVersionTooSmall() public
    {
        uint8 initialVersionInt = GIF_INITIAL_RELEASE().toInt();
        ...
        for(uint8 i = 0; i < 2; i++)
        {
            ...
            vm.expectRevert(abi.encodeWithSelector(
                ReleaseRegistry.ErrorReleaseRegistryServiceInfoReleaseMismatch.selector, 
                service,
                releaseVersion,
                serviceVersion
            ));
