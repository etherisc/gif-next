...
contract RewardCalculation is GifTest {
    ...
    function test_rewardCalculationConsoleLog() public {
        ...
        UFixed halfYearFraction = StakingLib.getYearFraction(halfYear);
    ...
    function test_rewardCalculationGetYearFraction() public {
        ...
        UFixed fractionFullYear = StakingLib.getYearFraction(yearDuration);
        ...
        UFixed fractionFourYears = StakingLib.getYearFraction(fourYearDuration);
        ...
        UFixed third = _third();
        UFixed fractionThirdYear = StakingLib.getYearFraction(thirdYearDuration);
    ...
    function test_rewardCalculationCalculateRewardInitial() public {
        ...
        Amount rewardAmount = StakingLib.calculateRewardAmount(
            tenPercentAPR, 
            yearDuration, 
            thousand);
    ...
    function test_rewardCalculationCalculateRewardApr() public {
        ...
        // 20% reward rate
        assertEq(
            StakingLib.calculateRewardAmount(UFixedLib.toUFixed(2, -1), yearDuration, thousand).toInt(), 
            200, "unexpected reward amount for (20%, 1 year, 1000)");

        // 0.1% reward rate
        assertEq(
            StakingLib.calculateRewardAmount(UFixedLib.toUFixed(1, -3), yearDuration, thousand).toInt(), 
    ...
    function test_rewardCalculationCalculateRewardDuration() public {
        ...
        // 5 years
        assertEq(
            StakingLib.calculateRewardAmount(tenPercentAPR, SecondsLib.toSeconds(5 * yearDurationInt), thousand).toInt(), 
            500, "unexpected reward amount for (10%, 5 year, 1000)");

        // 2 years
        assertEq(
            StakingLib.calculateRewardAmount(tenPercentAPR, SecondsLib.toSeconds(2 * yearDurationInt), thousand).toInt(), 
            200, "unexpected reward amount for (10%, 2 year, 1000)");

        // 1/2 year
        assertEq(
            StakingLib.calculateRewardAmount(tenPercentAPR, SecondsLib.toSeconds(yearDurationInt / 2), thousand).toInt(), 
            50, "unexpected reward amount for (10%, 1/2 year, 1000)");

        // 1/10 year
        assertEq(
            StakingLib.calculateRewardAmount(tenPercentAPR, SecondsLib.toSeconds(yearDurationInt / 10), thousand).toInt(), 
    ...
    function test_rewardCalculationCalculateRewardAmount() public {
        ...
        // 4000
        assertEq(
            StakingLib.calculateRewardAmount(tenPercentAPR, yearDuration, AmountLib.toAmount(4000)).toInt(), 
            400, "unexpected reward amount for (10%, 1 year, 4000)");

        // 1500
        assertEq(
            StakingLib.calculateRewardAmount(tenPercentAPR, yearDuration, AmountLib.toAmount(1500)).toInt(), 
            150, "unexpected reward amount for (10%, 1 year, 1500)");

        // 100
        assertEq(
            StakingLib.calculateRewardAmount(tenPercentAPR, yearDuration, AmountLib.toAmount(100)).toInt(), 
            10, "unexpected reward amount for (10%, 1 year, 100)");

        // 10
        assertEq(
            StakingLib.calculateRewardAmount(tenPercentAPR, yearDuration, AmountLib.toAmount(10)).toInt(), 
    ...
    function test_rewardCalculationCalculateRewardZero() public {
        ...
        // 0% reward rate
        assertEq(
            StakingLib.calculateRewardAmount(UFixedLib.toUFixed(0), yearDuration, thousand).toInt(), 
            0, "unexpected reward amount for (0%, 1 year, 1000)");

        // 0 seconds
        assertEq(
            StakingLib.calculateRewardAmount(tenPercentAPR, SecondsLib.toSeconds(0), thousand).toInt(), 
            0, "unexpected reward amount for (10%, 0 seconds, 1000)");

        // 0 amount
        assertEq(
            StakingLib.calculateRewardAmount(tenPercentAPR, yearDuration, AmountLib.zero()).toInt(), 
