...
contract RegisterRegistryFuzzTest is RegistryTestBase 
{
    ...
    function testFuzz_registryRegistry_withValidSenderAndNftId_00_longRunning(uint64 chainId, address registryAddress) public
    {
        registerRegistry_testFunction(
            gifAdmin,
            NftIdLib.toNftId(chainNft.calculateTokenId(registry.REGISTRY_TOKEN_SEQUENCE_ID(), chainId)),
            chainId,
            registryAddress
        );
    }
    ...
    function testFuzz_registryRegistry_withValidSenderAndNftId_10_longRunning(uint256 chainIdIdx, address registryAddress) public
    {
        registerRegistry_testFunction(
            gifAdmin,
            NftIdLib.toNftId(chainNft.calculateTokenId(registry.REGISTRY_TOKEN_SEQUENCE_ID(), _getChainIdAtIndex(chainIdIdx))),
            _getChainIdAtIndex(chainIdIdx),
            registryAddress
        );
    }
...
contract RegisterRegistryWithPresetFuzzTestL1 is RegisterRegistryWithPresetFuzzTest
{
    ...
    function testFuzz_registerRegistry_withDuplicateChainId_longRunning(address registryAddress) public
    {
        ...
        nftId = NftIdLib.toNftId(
            chainNft.calculateTokenId(registry.REGISTRY_TOKEN_SEQUENCE_ID(), 1)
        );
