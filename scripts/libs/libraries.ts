...
export async function deployLibraries(owner: Signer): Promise<LibraryAddresses> {
    ...
    LIBRARY_ADDRESSES.set("PoolLib", poolLibAddress);
    ...
    const { address: policyServiceLibAddress } = await deployContract(
        "PolicyServiceLib",
        owner,
        undefined,
        {
